...
def run(fname_templates_in,
    ...
    np.save(fname_templates_up, templates_up)
...
def run_reg_deconv(fname_templates_in,
                   output_directory,
                   reader,
                   max_iter,
                   upsample_max_val,
    ...
    logger.info('Number of Units IN: {}'.format(mp_object.temps_orig.shape[2]))
    ...
    return (mp_object.temps_orig.transpose(2,0,1),
            spike_train,
            templates_up.transpose(2,0,1),
            spike_train_up)
    
def run_deconv_with_updates(
    fname_templates_in,
    output_directory,
    reader,
    max_iter,
