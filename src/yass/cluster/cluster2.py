...
class Cluster(object):
    ...
    def cluster(self, current_indices, gen, branch, hist):
        ...
        # featurize #1
        pca_wf = self.featurize_step(gen, current_indices, current_indices)
        ...
        # run mfm
        vbParam = self.run_mfm(gen, pca_wf_subsample, weights)
        ...
        self.save_metadata(vbParam, pca_wf, current_indices,
                               gen, branch, hist)

        # single cluster
        if vbParam.rhat.shape[1] == 1:
            self.single_cluster_step(current_indices, pca_wf,
                                     gen, branch, hist)

        # multiple clusters
        else:
            self.multi_cluster_step(current_indices, pca_wf,
                                    vbParam, gen, branch, hist)

    def save_metadata(self, vbParam, pca_wf_all, current_indices,
                      gen, branch, hist):
        
        self.pca_post_triage_post_recovery.append(pca_wf_all)
        #self.vbPar_rhat.append(vbParam.rhat)
        #self.vbPar_muhat.append(vbParam.muhat)
    ...
    def load_data(self, data_in):
        ...
        self.max_n_spikes = self.CONFIG.cluster.max_n_spikes
    ...
    def initialize(self):
        ...
        self.indices_in = np.arange(len(self.spike_times_original))
    ...
    def load_waveforms(self):
        ...
        '''  Waveforms only loaded once in gen0 before clustering starts
        '''
    ...
    def featurize_step(self, gen, indices_to_feat, indices_to_transform):
    ...
    def recover_spikes(self, vbParam, pca):
    
        N, D = pca.shape
        threshold = np.sqrt(chi2.ppf(0.999, D))
    ...
    def single_cluster_step(self, current_indices, pca_wf,
                            gen, branch, hist):
    ...
    def multi_cluster_step(self, current_indices, pca_wf, vbParam,
                                gen, branch_current, hist):
        
        # this is outside of multi_cluster_step to make 
        
        cc_assignment, stability, idx_keep = self.get_cc_and_stability(vbParam)
        current_indices = current_indices[idx_keep]
        pca_wf = pca_wf[idx_keep]
        ...
        if len(stability) == 1:
            ...
            pca_wf = pca_subsampled = vbParam = None
            ...
            for branch_next, clust in enumerate(np.unique(cc_assignment)):
                ...
                self.cluster(current_indices[idx], gen+1, branch_next,
                             local_hist)
...
def coreset(data, m, K=4, delta=0.02):
