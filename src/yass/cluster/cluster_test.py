...
def connecting_points2(points, index, neighbors, keep=None):
    ...
    if keep[index] == 1:
        ...
        close_index = np.where(neighbors[points[index]][points])[0]

        for j in close_index:
            keep = connecting_points2(points, j, neighbors, keep)
