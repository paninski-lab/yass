...
#         self.fname_softassignment1 = fname_softassignment1
        ...
        self.ptps2, self.f_rates2 = get_ptp_firing_rates(self.fname_templates2, self.fname_spike_train2, self.reader)
...
#         plt.xlabel('Firing rates (Hz)', fontsize = fs)
        ...
        if use_ptp:
            plt.scatter(np.log(self.ptps[run1_only]),
                        np.log1p(self.avg_outlier1[run1_only]), s=ss, color='r')
            plt.scatter(np.log(self.ptps2[run2_only]),
                        np.log1p(self.avg_outlier2[run2_only]), s=ss, color='b')
        else:
            plt.scatter(np.log(self.f_rates[run1_only]),
                        np.log1p(self.avg_outlier1[run1_only]), s=ss, color='r')
            plt.scatter(np.log(self.f_rates2[run2_only]),
        ...
        plt.xlim([x_min-0.5, x_max+0.5])
        plt.ylim([np.log1p(outlier_min), np.log1p(outlier_max)])
...
#         plt.xlabel('Firing rates (Hz)', fontsize = fs)
        ...
        if use_ptp:
            plt.scatter(np.log(self.ptps[run1_matched]),
                        np.log1p(self.avg_outlier1[run1_matched]), s=ss, color='g')
            plt.scatter(np.log(self.ptps2[run2_matched]),
                        np.log1p(self.avg_outlier2[run2_matched]), s=ss, color='purple')
        else:
            plt.scatter(np.log(self.f_rates[run1_matched]),
                        np.log1p(self.avg_outlier1[run1_matched]), s=ss, color='g')
            plt.scatter(np.log(self.f_rates2[run2_matched]),
        ...
        name1_only = "YASS" + ' only'
        name2_only = "KS" + ' only'
        
        name1_matched = "YASS" + ' matched'
        name2_matched = "KS" + ' matched'
        ...
        if  use_ptp:
            plt.savefig(os.path.join(self.save_dir, 'figure_ptp.png'), facecolor='w')
        else:
            plt.savefig(os.path.join(self.save_dir, 'figure_fr.png'), facecolor='w')
