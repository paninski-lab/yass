"""
[Description of this files content]
"""
import logging
...
def getCleanSpikeTrain(config):
    """
        Threshold detection for extracting clean templates from the raw
        recording if groundtruth is not available

        Parameters:
        -----------
        config: configuration object
            configuration object containing the parameters for making the
            training data.

        Returns:
        -----------
        spikeTrain: np.array
            [number of spikes, 2] first column corresponds to spike time;
            second column corresponds to cluster id.
    """
...
class AugmentedSpikes(object):
    """
    Class for making the training data for the neural network detector,
    the autoencoder and the triage network

    Attributes:
    -----------
    config: configuration object
        configuration object containing the parameters for making the training
        data.

    spikeTrain: np.array
        [number of spikes, 2] first column corresponds to spike time; second
        column corresponds to cluster id.
    """
    ...
    def __init__(self, *args):
        """
            ...
            Parameters:
            -----------
            args: if no spiketrain is input, threshold detection will be
            performed to obtain a spiketrain from the recording
    ...
    def getBigTemplates(self, R):
        """
            ...
            -----------
            templates: np.array
                [number of templates, temporal length, number of channels]
                returned templates.
        """
    ...
    def determineNoiseCov(self, temporal_size, D):
        """
            ...
            D: int
                number of channels away from the mainchannel. D=1 if only look
                at the main channel; D=2 if look at the main
                channel and its neighboring channels; D=3 if look at the main
                channel, the neighboring channels and the
                neighbors of the neighboring channels.
            Returns:
            -----------
            spatial_SIG: np.array
                [d, d] spatial covariance matrix of the noise where d depends
                on D.

            temporal_SIG: np.array
                [temporal_size, temporal_size] temporal covariance matrix of
                the noise.
        """
        pp = Preprocessor(self.config)
    ...
    def make_training_data(self, R, D, k_idx, min_amp, nspikes):
        """
        Makes the training data for the neural network detector, the
        autoencoder and the triage network
        ...
        D: int
            number of channels away from the mainchannel. D=1 if only look at
            the main channel; D=2 if look at the main
            channel and its neighboring channels; D=3 if look at the main
            channel, the neighboring channels and the neighbors
            of the neighboring channels.
        k_idx: np.array
            [ncluster,] indices of the clusters whose waveforms will be used
            for making the training data. k_idx could
            be a (real) subset of the whole cluster indices if some of the
            clusters have low energy and bad shapes and
            should not enter the function.
        min_amp: float
            minimum number for the maximum amplitude of the spike on the main
            channel for all the generated augmented spikes.
            This could be defined by visual inspection. Empirically
            min_amp = 4 works well for most of the cases.
        nspikes: int
            number of spikes to be generated. Notice that this is not
            necessarily the total number of the output training
            data (see Returns for details).

        Returns:
        -----------
        x_detect: np.array
            [5*nspikes, 2*R+1, d] training data for the neural network
            detector where d depends on D. This training data
            contains isolated spikes, noise, colliding spikes and misaligned
            spikes. The channels are ordered with respect
            to their distance to the first channel (x_detect[:,:,0]).
        y_detect: np.array
            [5*nspikes,1] label for x_detect, 1 for isolated spikes and
            collisions and 0 otherwise.
        x_triage: np.array
            [3*nspikes, 2*R+1, d] training data for the triage network. This
            training data contains isolated spikes and colliding spikes.
        y_triage: np.array
            [5*nspikes,1] label for x_triage, 1 for isolated spikes and 0
            otherwise.
        x_ae: np.array
            [3*nspikes, 2*R+1] training data for the autoencoder. This
            training data contains noisy isolated spikes.
        y_ae: np.array
            [3*nspikes, 2*R+1] training data for the autoencoder. This
            training data contains clean isolated spikes (isolated
        ...
        templatesBig2, geom, neighChannels = self.crop_templates(
            templatesBig, R, D+1)
        ...
        x_clean = np.zeros(
            (nk*K, templatesBig2.shape[1], templatesBig2.shape[2]))
        for k in range(K):
            tt = templatesBig2[k]
        ...
        for j in range(nk*K):
            ...
            idx_candidate = np.where(np.logical_and(amp_per_data
                                                    > amp_per_data[j]*0.5,
                                                    amp_per_data
                                                    < amp_per_data[j]*2))[0]
            ...
            x_clean2 = np.copy(x_clean[idx_match]
                                      [:, np.random.choice(nneigh,
                                                           nneigh,
                                                           replace=False)])

            if shift > 0:
                x_collision[j, :(x_collision.shape[1]-shift)
                            ] += x_clean2[shift:]
        ...
        for j in range(nk*K):
            ...
            if shift > 0:
                x_collision2[j, :(x_collision.shape[1]-shift)
                             ] += x_clean2[shift:]
        ...
        for j in range(nk*K):
            ...
            if shift > 0:
                x_collision3[j, :(x_collision.shape[1]-shift)
                             ] += x_clean2[shift:]
        ...
        for j in range(x_ae.shape[0]):
            ...
            x_ae[j] = x_clean[j, (t_mid-R+temporal_shifts_ae[j]):(t_mid+R+1+temporal_shifts_ae[j]), 0]+noise[
                j, (t_mid-R+temporal_shifts_ae[j]):(t_mid+R+1+temporal_shifts_ae[j]), 0]
    ...
    def crop_templates(self, templatesBig, R, D):
        ...
        for k in range(K):
            ...
            if shift > 0:
                ...
                templates_mainc[
                    k, (-shift):] = t1[:(templatesBig.shape[1]+shift)]
                templatesBig[k, (-shift):] = templatesBig[k,
                                                          :(templatesBig.shape[1]+shift)]
        ...
        neighChannels = (squareform(pdist(geom)) <=
                         self.config.recordings.spatial_radius)

        templatesBig2 = np.zeros(
            (templatesBig.shape[0], templatesBig.shape[1], nChannels))
        for k in range(K):
            ch_idx = np.where(neighChanBig[mainC[k]])[0]
            ch_idx, temp = order_channels_by_distance(
                mainC[k], ch_idx, self.config.geom)
