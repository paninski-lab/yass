...
def nn_detection(X, T_batch, buff, neighChannels, geom,
    ...
    local_max_idx_tf = nnd.get_spikes(
        x_tf, T_small+2*buff, nneigh, c_idx, temporal_window, th_detect)
    ...
    spike_index_tf = remove_duplicate_spikes_by_energy(
        energy_train_tf, T_small+2*buff, c_idx, temporal_window)
    ...
    with tf.Session() as sess:
        ...
        for j in range(nbatches):
            ...
            spike_index_clear_all[counter_clear:(
                counter_clear+n_clean)] = spike_index_clear
            spike_index_collision_all[counter_collision:(
                counter_collision+n_col)] = spike_index_collision
