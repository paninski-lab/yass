...
class Deconvolution(object):
    ...
    def fullMPMU(self):
        ...
        self.logger.debug('wfile is {} {}'
                          .format(self.path_to_file,
                                  os.path.getsize(self.path_to_file)))
        nBatches = self.config.nBatches
        flattenedLength = (2*(self.config.batch_size + 2*self.config.BUFF)
        ...
        for i in range(nBatches):
            ...
            spike_index_batch[:, 0] = spike_index_batch[
                :, 0] - self.config.batch_size*i + self.config.BUFF
