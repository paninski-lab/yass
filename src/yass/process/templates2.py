...
def get_templates(spike_train_clear, batch_size, buff, n_batches, n_channels,
    ...
    neighchan = n_steps_neigh_channels(neighbors, steps=3)
    K = np.max(spike_train_clear[:, 0])+1
    ...
    for i in range(n_batches):
        ...
        spikeTrain_batch = spike_train_clear[spike_train_clear[:, 3] == i]

        for c in range(n_channels):
            spt_c = spikeTrain_batch[spikeTrain_batch[:, 1] == c, 2]
            L_c = spikeTrain_batch[spikeTrain_batch[:, 1] == c, 0]

            if spt_c.shape[0] > 0:
                ch_idx = np.where(neighchan[c])[0]
                wf = np.zeros(
                    (spt_c.shape[0], templates.shape[1], ch_idx.shape[0]))

                for j in range(spt_c.shape[0]):
                    wf[j] = wrec[
                        spt_c[j]+np.arange(-(spike_size+template_max_shift),
                                           spike_size+template_max_shift+1)][:, ch_idx]

                for k in range(K):
                    templates[ch_idx, :,
                              k] += np.sum(wf[L_c == k], axis=0).T
                    weights[k] += np.sum(L_c == k)
    ...
    denoiser = np.matmul(proj,proj.T)
    for k in range(templates.shape[2]):
        templates[:,:,k] = np.matmul(templates[:,:,k],denoiser)
    
    Knew = len(tempGroups)
    L_old = spike_train_clear[:, 0]
...
def mergeTemplates(templates, weights, neighbors, template_max_shift,
    ...
    for k1 in range(K):
        ...
        for k2 in k2s:
            ...
            if TemplatesSimilarity(t1, t2, th, W, proj):
...
def TemplatesSimilarity(t1, t2, th, W, proj):
    C, RW = t1.shape
    R = RW - 2*W

    denoiser = np.matmul(proj,proj.T)
    
    t1 = np.reshape(np.matmul(t1[:, W:(W+R)],denoiser), R*C)
    t2_shifted = np.zeros((2*W+1, R*C))
    for j in range(2*W+1):
        t2_shifted[j] = np.reshape(np.matmul(t2[:, j:(j+R)],denoiser), R*C)
